			       ...
			       struct sockaddr *addr, socklen_t addrlen)
{
	...
	if (pollfd->fd < 0) {
		ret = -errno;
		LOG_ERROR("Failed to create telnet AF_INET%s socket",
			family == AF_INET ? "" : "6");
		goto error;
	}
	...
	if (zsock_bind(pollfd->fd, addr, addrlen) < 0) {
		ret = -errno;
		LOG_ERROR("Cannot bind on family AF_INET%s (%d)",
	...
	if (zsock_listen(pollfd->fd, 1)) {
		ret = -errno;
		LOG_ERROR("Cannot listen on family AF_INET%s (%d)",
			family == AF_INET ? "" : "6", ret);
		goto error;
	}
...
error:
	LOG_ERROR("Unable to start telnet on AF_INET%s (%d)",
		family == AF_INET ? "" : "6", ret);
