...
static struct usbd_context *corelynx_usbd;

static int enable_usb_device_next(void)
{
	int err;

	corelynx_usbd = sample_usbd_init_device(NULL);
	if (corelynx_usbd == NULL) {
		return -ENODEV;
	}

	err = usbd_enable(corelynx_usbd);
	if (err) {
		return err;
	}
...
SYS_INIT(enable_usb_device_next, POST_KERNEL, 95);
