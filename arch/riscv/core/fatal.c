					   ...
					   const _callee_saved_t *csf)
{
	...
	LOG_LASTWORD("");
	LOG_LASTWORD(" mcause: %ld, %s", mcause, z_riscv_mcause_str(mcause));

#ifndef CONFIG_SOC_OPENISA_RV32M1
	unsigned long mtval;

	__asm__ volatile("csrr %0, mtval" : "=r" (mtval));
	LOG_LASTWORD("  mtval: %lx", mtval);
#endif /* CONFIG_SOC_OPENISA_RV32M1 */

#ifdef CONFIG_EXCEPTION_DEBUG
	if (esf != NULL) {
		LOG_LASTWORD("     a0: " PR_REG "    t0: " PR_REG, esf->a0, esf->t0);
		LOG_LASTWORD("     a1: " PR_REG "    t1: " PR_REG, esf->a1, esf->t1);
		LOG_LASTWORD("     a2: " PR_REG "    t2: " PR_REG, esf->a2, esf->t2);
#if defined(CONFIG_RISCV_ISA_RV32E)
		LOG_LASTWORD("     a3: " PR_REG, esf->a3);
		LOG_LASTWORD("     a4: " PR_REG, esf->a4);
		LOG_LASTWORD("     a5: " PR_REG, esf->a5);
#else
		LOG_LASTWORD("     a3: " PR_REG "    t3: " PR_REG, esf->a3, esf->t3);
		LOG_LASTWORD("     a4: " PR_REG "    t4: " PR_REG, esf->a4, esf->t4);
		LOG_LASTWORD("     a5: " PR_REG "    t5: " PR_REG, esf->a5, esf->t5);
		LOG_LASTWORD("     a6: " PR_REG "    t6: " PR_REG, esf->a6, esf->t6);
		LOG_LASTWORD("     a7: " PR_REG, esf->a7);
#endif /* CONFIG_RISCV_ISA_RV32E */
		LOG_LASTWORD("     sp: " PR_REG, z_riscv_get_sp_before_exc(esf));
		LOG_LASTWORD("     ra: " PR_REG, esf->ra);
		LOG_LASTWORD("   mepc: " PR_REG, esf->mepc);
		LOG_LASTWORD("mstatus: " PR_REG, esf->mstatus);
		LOG_LASTWORD("");
	}

	if (csf != NULL) {
#if defined(CONFIG_RISCV_ISA_RV32E)
		LOG_LASTWORD("     s0: " PR_REG, csf->s0);
		LOG_LASTWORD("     s1: " PR_REG, csf->s1);
#else
		LOG_LASTWORD("     s0: " PR_REG "    s6: " PR_REG, csf->s0, csf->s6);
		LOG_LASTWORD("     s1: " PR_REG "    s7: " PR_REG, csf->s1, csf->s7);
		LOG_LASTWORD("     s2: " PR_REG "    s8: " PR_REG, csf->s2, csf->s8);
		LOG_LASTWORD("     s3: " PR_REG "    s9: " PR_REG, csf->s3, csf->s9);
		LOG_LASTWORD("     s4: " PR_REG "   s10: " PR_REG, csf->s4, csf->s10);
		LOG_LASTWORD("     s5: " PR_REG "   s11: " PR_REG, csf->s5, csf->s11);
#endif /* CONFIG_RISCV_ISA_RV32E */
		LOG_LASTWORD("");
	}
