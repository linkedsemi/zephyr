...
struct ecc_p384_ops {
	int (*sign)(struct ecc_p384_ctx *ctx, struct ecc_p384_key *key, struct ecc_p384_pkt *pkt); //uint8_t *digest, 
	int (*verify)(struct ecc_p384_ctx *ctx, struct ecc_p384_key *key, struct ecc_p384_pkt *pkt);
	int (*keygen)(struct ecc_p384_ctx *ctx, struct ecc_p384_key *key);
};
...
/* The API a ecc_p384 driver should implement */
__subsystem struct ecc_p384_driver_api {
	int (*query_hw_caps)(const struct device *dev);

	/* Setup a ecdas session */
	int (*begin_session)(const struct device *dev, struct ecc_p384_ctx *ctx,
				 struct ecc_p384_key *key);

	/* Tear down an established session */
	int (*free_session)(const struct device *dev, struct ecc_p384_ctx *ctx);
};
...
/**
 * @brief Setup a ecc_p384 session
 *
 ...
 */
static inline int ecc_p384_begin_session(const struct device *dev,
					struct ecc_p384_ctx *ctx, struct ecc_p384_key *key)
{
	struct ecc_p384_driver_api *api;

	api = (struct ecc_p384_driver_api *)dev->api;
	ctx->device = dev;
...
/**
 * @brief Cleanup a ecc_p384 session
 *
 * Clears the hardware and/or driver state of a previous session.
 *
 * @param  dev      Pointer to the device structure for the driver instance.
 * @param  ctx      Pointer to the ecc_p384 context structure of the session
 *			to be freed.
 *
 * @return 0 on success, negative errno code on fail.
 */
static inline int ecc_p384_free_session(const struct device *dev,
					   struct ecc_p384_ctx *ctx)
{
	struct ecc_p384_driver_api *api;
...
/**
 * @brief Perform ecc_p384 sign.
 *
 * @param  ctx   Pointer to the ecc_p384 context of this op.
 * @param  pkt   Structure holding the input/output buffer pointers.
 *
 * @return 0 on success, negative errno code on fail.
 */
static inline int ecc_p384_sign(struct ecc_p384_ctx *ctx,struct ecc_p384_key *key,
				   struct ecc_p384_pkt *pkt)
{
	pkt->ctx = ctx;
 ...
 * @return 0 on success, negative errno code on fail.
 */
static inline int ecc_p384_verify(struct ecc_p384_ctx *ctx,struct ecc_p384_key *key,
				 struct ecc_p384_pkt *pkt)
{
	pkt->ctx = ctx;
...
/**
 * @brief Perform genarater ecc_p384 private/public key pair.
 *
 * @param  ctx   Pointer to the ecc_p384 context of this op.
 * @param  pkt   Structure holding the input/output buffer pointers.
 *
 * @return 0 on success, negative errno code on fail.
 */
static inline int ecc_p384_keygen(struct ecc_p384_ctx *ctx,
				 struct ecc_p384_key *key)
{
