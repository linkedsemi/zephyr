...
struct k_mutex cpu_domain_lock[PECI_OFFSET_MAX][DOMAIN_OFFSET_MAX];

// static uint total_retry_timeout_us __read_mostly;
static uint total_retry_timeout_us ;
...
static uint retry_interval_min_us  = PECI_DEV_RETRY_INTERVAL_MIN_USEC;
...
static uint retry_interval_max_us  = PECI_DEV_RETRY_INTERVAL_MAX_USEC;
...
static inline uint64_t le64_to_cpup(uint64_t* val) {
    return sys_le64_to_cpu(*val);
}
static inline uint16_t le16_to_cpup(uint16_t* val) {
...
bool object_is_on_stack(void *ptr) {
    return (ptr >= (void *)&__stack) && (ptr < (void *)&__heap);
}

extern uint8_t __heap_start[]; 
extern uint8_t __heap_end[]; 

bool is_vmalloc_addr(void *addr) {
    return (addr >= (void *)__heap_start) && (addr < (void *)__heap_end);
}
...
struct peci_xfer_msg *peci_get_xfer_msg(u8 tx_len, u8 rx_len)
{
	...
	msg->rx_len = rx_len;
	msg->rx_buf = rx_buf;
	// LOG_DBG("Debug in %s: msg->rx_buf = %p to %p\n", __func__, msg->rx_buf, msg->rx_buf+rx_len-1);
	// LOG_DBG("Debug in %s: msg->tx_buf = %p to %p\n", __func__, msg->tx_buf, msg->tx_buf+tx_len-1);
	// LOG_DBG("Debug in %s: msg = %p to %p\n", __func__, msg, msg+sizeof(*msg)-1);
	// LOG_DBG("Debug in %s: msg size is p = %d and s = %d\n", __func__, sizeof(*msg), sizeof(struct peci_xfer_msg));

	return msg;

err_free_tx_rx_buf:
	// LOG_DBG("Debug in %s: in err_free_tx_rx_buf");
	free(rx_buf);
err_free_tx_buf:
	// LOG_DBG("Debug in %s: in err_free_tx_buf");
	free(tx_buf);
...
void peci_put_xfer_msg(struct peci_xfer_msg *msg)
{
	...
	free(msg->rx_buf);
	msg->rx_buf = NULL;
	// LOG_DBG("Debug in %s: free msg->rx_buf\n", __func__);
	// LOG_DBG("Debug in %s: msg->tx_buf = %p\n", __func__, msg->tx_buf);
	free(msg->tx_buf);
	// LOG_DBG("Debug in %s: free msg->tx_buf\n", __func__);
	msg->tx_buf = NULL;
	// LOG_DBG("Debug in %s: free msg->tx_buf\n", __func__);
	// LOG_DBG("Debug in %s: msg = %p\n", __func__, msg);
	free(msg);
	msg = NULL;
	// LOG_DBG("Debug in %s: free msg\n", __func__);
}
...
static int peci_aw_fcs(struct peci_xfer_msg *msg, int len, u8 *aw_fcs)
{
	...
	*aw_fcs = crc8_lx(peci_crc8_table, tmp_buf, (size_t)len, 0);
		       ...
		       bool do_retry, bool has_aw_fcs)
{
	...
	ulong timeout = sys_clock_tick_get_32();
	...
	if (do_retry)
		timeout += k_us_to_ticks_ceil32(total_retry_timeout_us);
	...
	// LOG_DBG("Debug in %s: before loop\n", __func__);
	for (;;) {
		// LOG_DBG("Debug in %s: before k_mutex_lock\n", __func__);
		k_mutex_lock(&cpu_domain_lock[node_id][domain_id], K_FOREVER);
		// LOG_DBG("Debug in %s: before adapter->xfer\n", __func__);
		ret = adapter->xfer(adapter, msg);
		k_mutex_unlock(&cpu_domain_lock[node_id][domain_id]);
		// LOG_DBG("Debug in %s: finish adapter->xfer\n", __func__);
		...
		if (sys_clock_tick_get_32() - timeout < 0) {
...
static int peci_scan_cmd_mask(struct peci_adapter *adapter)
{
	...
	if (adapter->cmd_mask & BIT(PECI_CORE_CMD_XFER))
		return 0;
	...
	// LOG_DBG("Debug in %s: before peci_xfer\n", __func__);
	...
	if (revision >= 0x41) { /* Rev. 4.1 */
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_TELEMETRY_DISC);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_TELEMETRY_GET_TELEM_SAMPLE);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_TELEMETRY_CONFIG_WATCHER_RD);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_TELEMETRY_CONFIG_WATCHER_WR);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_TELEMETRY_GET_CRASHLOG_SAMPLE);
	}
	if (revision >= 0x40) { /* Rev. 4.0 */
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_RD_IA_MSREX);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_RD_END_PT_CFG);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_WR_END_PT_CFG);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_CRASHDUMP_DISC);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_CRASHDUMP_GET_FRAME);
	}
	if (revision >= 0x36) /* Rev. 3.6 */
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_WR_IA_MSR);
	if (revision >= 0x35) /* Rev. 3.5 */
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_WR_PCI_CFG);
	if (revision >= 0x34) /* Rev. 3.4 */
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_RD_PCI_CFG);
	if (revision >= 0x33) { /* Rev. 3.3 */
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_RD_PCI_CFG_LOCAL);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_WR_PCI_CFG_LOCAL);
	}
	if (revision >= 0x32) /* Rev. 3.2 */
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_RD_IA_MSR);
	if (revision >= 0x31) { /* Rev. 3.1 */
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_RD_PKG_CFG);
		adapter->cmd_mask |= BIT(PECI_CORE_CMD_WR_PKG_CFG);
	}

	adapter->cmd_mask |= BIT(PECI_CORE_CMD_XFER);
	adapter->cmd_mask |= BIT(PECI_CORE_CMD_GET_TEMP);
	adapter->cmd_mask |= BIT(PECI_CORE_CMD_GET_DIB);
	adapter->cmd_mask |= BIT(PECI_CORE_CMD_PING);
...
static int peci_check_cmd_support(struct peci_adapter *adapter,
				  enum peci_cmd cmd)
{
	if (!(adapter->cmd_mask & BIT(PECI_CORE_CMD_PING)) &&
...
static int peci_cmd_ping(struct peci_adapter *adapter, uint msg_len, void *vmsg)
{
	...
	// LOG_DBG("Debug in %s: before peci_xfer\n", __func__);
	ret = peci_xfer(adapter, msg);
	// LOG_DBG("Debug in %s: finish peci_xfer\n", __func__);
	peci_put_xfer_msg(msg);
...
static int peci_cmd_get_dib(struct peci_adapter *adapter, uint msg_len, void *vmsg)
{
	...
	ret = peci_xfer(adapter, msg);
	// LOG_DBG("Debug in %s: peci_xfer ret = %p\n", __func__, &ret);
	if (ret)
		goto out;

	// LOG_DBG("Debug in %s: peci_cmd_get_dib umsg->dib = %p\n", __func__, &umsg->dib);
	// LOG_DBG("Debug in %s: peci_cmd_get_dib umsg->dib = %llx\n", __func__, umsg->dib);
...
static int peci_cmd_get_temp(struct peci_adapter *adapter, uint msg_len, void *vmsg)
{
	...
	// LOG_DBG("Debug in %s: peci_cmd_get_temp umsg->temp_raw = %d\n", __func__, umsg->temp_raw);
...
static const peci_cmd_fn_type peci_cmd_fn[PECI_CORE_CMD_MAX] = {
	peci_cmd_xfer,
	peci_cmd_ping,
	peci_cmd_get_dib,
	peci_cmd_get_temp,
...
int peci_command(struct peci_adapter *adapter, enum peci_cmd cmd, uint msg_len, void *vmsg)
{
	...
	LOG_DBG("Debug: %s, cmd=0x%02x\n", __func__, cmd);
	LOG_DBG("Debug: adapter is used in %p\n", (void *)adapter);
	LOG_DBG("Debug: adapter bus lock is used in %p\n", (void *)&adapter->bus_lock);
	k_mutex_lock(&adapter->bus_lock, K_FOREVER);
	LOG_DBG("Debug in %s: finish k_mutex_lock\n", __func__);

	ret = peci_check_cmd_support(adapter, cmd);
	LOG_DBG("Debug in %s: finished peci_check_cmd_support ret = %d\n", __func__, ret);
	if (!ret){
		LOG_DBG("Debug in %s: before peci_cmd_fn\n", __func__);
		ret = peci_cmd_fn[cmd](adapter, msg_len, vmsg);
		LOG_DBG("Debug in %s: finish peci_cmd_fn ret = %d\n", __func__, ret);
	}

	LOG_DBG("Debug in %s: before k_mutex_unlock\n", __func__);
	k_mutex_unlock(&adapter->bus_lock);
	LOG_DBG("Debug in %s: finish k_mutex_unlock\n", __func__);
	return ret;
}
...
// 	client = k_calloc(sizeof(*client), GFP_KERNEL);
// 	if (!client)
...
static int peci_register_adapter(struct peci_adapter *adapter)
{
	...
	LOG_DBG("Debug: In peci_register_adapter\n");

	/* Can't register until after driver model init */
	if (!is_registered)
	{
		LOG_DBG("peci adapter is not registered yet\n");
		goto err_free_idr;
	}

	if (!adapter->name[0])
	{
		LOG_DBG("peci adapter has no name\n");
		goto err_free_idr;
	}	

	if (!adapter->xfer)
	{
		LOG_DBG("peci adapter has no xfer function\n");
		goto err_free_idr;
	}
		
	k_mutex_init(&adapter->bus_lock);
	LOG_DBG("Debug: adapter bus lock is init in %p\n", (void *)&adapter->bus_lock);
	k_mutex_init(&adapter->userspace_clients_lock);
	LOG_DBG("Debug in %s: after init userspace_clients_lock\n", __func__);
