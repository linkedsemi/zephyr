...
static int cmd_tck_run(const struct shell *shell, size_t argc, char **argv)
{
	...
	run_count = strtoul(argv[1], NULL, 0);

	err = jtag_tck_run(dev, run_count);

	if (err) {
		shell_error(shell, "failed to get JTAG tck_run (err %d)", err);
		return err;
	}
...
const char *tap_state_to_string(enum tap_state state)
{
    switch (state) {
        case TAP_INVALID: return "TAP_INVALID";
        case TAP_DREXIT2: return "TAP_DREXIT2";
		case TAP_DREXIT1: return "TAP_DREXIT1";
        case TAP_DRSHIFT: return "TAP_DRSHIFT";
        case TAP_DRPAUSE: return "TAP_DRPAUSE";
        case TAP_IRSELECT: return "TAP_IRSELECT";
        case TAP_DRUPDATE: return "TAP_DRUPDATE";
        case TAP_DRCAPTURE: return "TAP_DRCAPTURE";
        case TAP_DRSELECT: return "TAP_DRSELECT";
        case TAP_IREXIT2: return "TAP_IREXIT2";
        case TAP_IREXIT1: return "TAP_IREXIT1";
        case TAP_IRSHIFT: return "TAP_IRSHIFT";
        case TAP_IRPAUSE: return "TAP_IRPAUSE";
        case TAP_IRUPDATE: return "TAP_IRUPDATE";
        case TAP_IRCAPTURE: return "TAP_IRCAPTURE";
        case TAP_RESET: return "TAP_RESET";
        default: return "UNKNOWN";
    }
}

static int cmd_tap_get(const struct shell *shell, size_t argc, char **argv)
{
	const struct device *dev;
	int err;
	...
	err = jtag_tap_get(dev, &state);

	if (err) {
		shell_error(shell, "failed to get JTAG tap_get (err %d)", err);
		return err;
	}else{
		const char *state_str = tap_state_to_string(state);
    	shell_print(shell, "%s", state_str);
	}

	return 0;
}

static int cmd_tdo_get(const struct shell *shell, size_t argc, char **argv)
{
	const struct device *dev;
	int err;
	uint8_t value;
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(tck_run, NULL, "<run_count>", cmd_tck_run, 2,
			  0),
	SHELL_CMD_ARG(tap_get, NULL, "<NULL>", cmd_tap_get, 1,
			  0),
	SHELL_CMD_ARG(tdo_get, NULL, "<NULL>", cmd_tdo_get, 1,
			  0),
