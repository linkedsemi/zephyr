...
static inline enum clock_control_status ls_clock_control_get_status(const struct device *dev,
								 clock_control_subsys_t sub_system)
{
	struct ls_clk_cfg *clk_cfg = (struct ls_clk_cfg *)(sub_system);
	const struct cctl_ls_cfg *const config = dev->config;
	if(LS_CLK_SET(config->reg, clk_cfg->cctl_addr_offest) & BIT(clk_cfg->set_bit))
	{
