				       ...
				       const struct device *dev)
{
	...
	{
		const struct gpio_ls_config *cfg = dev->config;
		awo_io_reg_t *gpio = (awo_io_reg_t *)cfg->base;
	...
	{
		struct gpio_ls_common_data *data = (struct gpio_ls_common_data*)DEVICE_DT_GET(DT_INST(0, linkedsemi_ls_pinctrl))->data;
		const struct device *port_dev;
		struct gpio_ls_data *port_data;
	    uint32_t interrupt_status = 0;
		volatile uint32_t *INT_STAT_BASE = SYSC_AWO->EXTI_STT;
		volatile uint32_t *INT_CLR_BASE = SYSC_AWO->EXTI_CLR;
	...
	static void gpio_v33_isr(const struct device *dev)
	{
		struct gpio_ls_common_data *data = (struct gpio_ls_common_data*)DEVICE_DT_GET(DT_INST(0, linkedsemi_ls_pinctrl))->data;
		const struct device *port_dev;
		struct gpio_ls_data *port_data;
	    uint32_t interrupt_status = 0;
		volatile uint32_t *INT_STAT_BASE = V33_RG->GPIO_INTR_STT;
		volatile uint32_t *INT_CLR_BASE = V33_RG->GPIO_INTR_CLR;
	    for (uint8_t i = 0; i < 8; ++i) {
			volatile uint32_t *INT_STAT_REG = &INT_STAT_BASE[i];
	        volatile uint32_t *INT_CLR_REG = &INT_CLR_BASE[i];
		    uint32_t int_stat = *INT_STAT_REG;
							      ...
							      enum gpio_int_trig trig)
	{
		...
		if(pinval==0x17 || pinval==0x18 || pinval==0x19 || pinval==0x1a || pinval==0x24 || pinval==0x30 || pinval==0x31 || pinval==0x39
		|| pinval==0x3a || pinval==0x3b || pinval==0x3c || pinval==0x3d || pinval==0x3e || pinval==0x3f || pinval==0x6e || pinval==0x6f)
		{
			io_vcore_exti_config(pinval,config);
			return 0;
		}
		io_v33_exti_config(pinval,config);
		return 0;
	}
	...
	static int gpio_ls_common_init(const struct device *dev) 
	{
	    ...
	    IRQ_CONNECT(78,
			    DT_IRQ_BY_NAME(DT_INST(0, linkedsemi_ls_pinctrl), exti_gpio_v33, priority),
			    gpio_vcore_isr,
			    NULL, 
				0);
	    ...
	    IRQ_CONNECT(19,
			    DT_IRQ_BY_NAME(DT_INST(0, linkedsemi_ls_pinctrl), exti_gpio_vcore, priority),
			    gpio_v33_isr,
			    NULL, 
				0);
