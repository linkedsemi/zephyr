...
#define DT_DRV_COMPAT linkedsemi_sgpiom
...
LOG_MODULE_REGISTER(sgpiom_linkedsemi);
...
struct sgpiom_linkedsemi_parent_config {
...
uint16_t gpio_offset_gather[] = {
    ...
    [4] = offsetof(struct sgpiom_register_s, gather4),
    [5] = offsetof(struct sgpiom_register_s, gather5),
    [6] = offsetof(struct sgpiom_register_s, gather6),
    [7] = offsetof(struct sgpiom_register_s, gather7),
};

/* Driver convenience defines */
#define DEV_PARENT_CFG(dev) ((const struct sgpiom_linkedsemi_parent_config *)(dev)->config)
#define DEV_CFG(dev)        ((const struct sgpiom_linkedsemi_config *)(dev)->config)
#define DEV_DATA(dev)       ((struct sgpiom_linkedsemi_data *)(dev)->data)

static void sgpiom_linkedsemi_isr(const void *arg)
{
    const struct device *parent = arg;
    const struct sgpiom_linkedsemi_parent_config *cfg;
    const struct device *dev;
    struct sgpiom_linkedsemi_data *data;
    ...
    for (index = 0; index < cfg->child_num; index++) {
        ...
        gather_reg = (struct sgpiom_gather_register_s *)((uint32_t)DEV_CFG(dev)->base + gpio_offset_gather[group_idx]);
...
/* GPIO api functions */
static int sgpiom_linkedsemi_port_get_raw(const struct device *dev,
...
static int sgpiom_linkedsemi_port_set_masked_raw(const struct device *dev,
...
static int sgpiom_linkedsemi_port_set_bits_raw(const struct device *dev,
                                               gpio_port_value_t mask)
{
    return sgpiom_linkedsemi_port_set_masked_raw(dev, mask, mask);
}

static int sgpiom_linkedsemi_port_clear_bits_raw(const struct device *dev,
                                                 gpio_port_value_t mask)
{
    return sgpiom_linkedsemi_port_set_masked_raw(dev, mask, 0);
}

static int sgpiom_linkedsemi_port_toggle_bits(const struct device *dev,
                                              gpio_port_value_t mask)
{
    uint8_t pin_offset = DEV_CFG(dev)->pin_offset;
    struct sgpiom_register_s *gpio_reg = DEV_CFG(dev)->base;
...
static int sgpiom_linkedsemi_pin_interrupt_configure(const struct device *dev,
                                                     gpio_pin_t pin,
    ...
    if (mode == GPIO_INT_MODE_DISABLED) {
        ...
        if (mode == GPIO_INT_MODE_LEVEL) {
            if (trig == GPIO_INT_TRIG_LOW) {
                int_type = LINKEDSEMI_SGPIOM_LEVEL_LOW;
            } else if (trig == GPIO_INT_TRIG_HIGH) {
                int_type = LINKEDSEMI_SGPIOM_LEVEL_HIGH;
            } else {
                return -ENOTSUP;
            }
        } else {
            if (trig == GPIO_INT_TRIG_LOW) {
                int_type = LINKEDSEMI_SGPIOM_FALLING_EDGE;
            } else if (trig == GPIO_INT_TRIG_HIGH) {
                int_type = LINKEDSEMI_SGPIOM_RAISING_EDGE;
            } else {
                int_type = LINKEDSEMI_SGPIOM_DUAL_EDGE;
            }
        }
    }
...
static int sgpiom_linkedsemi_manage_callback(const struct device *dev,
                                             struct gpio_callback *callback,
                                             bool set)
{
    struct sgpiom_linkedsemi_data *data = DEV_DATA(dev);

    return gpio_manage_callback(&data->cb, callback, set);
}

static int sgpiom_linkedsemi_config(const struct device *dev,
                                    gpio_pin_t pin,
                                    gpio_flags_t flags)
{
    int ret = 0;
    ...
    if (flags & GPIO_OUTPUT) {
        ...
        if (flags & GPIO_OUTPUT_INIT_HIGH) {
            ret = sgpiom_linkedsemi_port_set_bits_raw(dev, BIT(pin));
        } else if (flags & GPIO_OUTPUT_INIT_LOW) {
            ret = sgpiom_linkedsemi_port_clear_bits_raw(dev, BIT(pin));
        }
...
__maybe_unused static int sgpiom_linkedsemi_passthrough(const struct device *dev, gpio_port_pins_t mask)
{
    gpio_port_value_t val;

    sgpiom_linkedsemi_port_get_raw(dev, &val);
    sgpiom_linkedsemi_port_set_masked_raw(dev, mask, val);

    return 0;
}

/* GPIO driver registration */
static const struct gpio_driver_api sgpiom_linkedsemi_driver = {
    .pin_configure = sgpiom_linkedsemi_config,
    .port_get_raw = sgpiom_linkedsemi_port_get_raw,
    .port_set_masked_raw = sgpiom_linkedsemi_port_set_masked_raw,
    .port_set_bits_raw = sgpiom_linkedsemi_port_set_bits_raw,
    .port_clear_bits_raw = sgpiom_linkedsemi_port_clear_bits_raw,
    .port_toggle_bits = sgpiom_linkedsemi_port_toggle_bits,
    .pin_interrupt_configure = sgpiom_linkedsemi_pin_interrupt_configure,
    .manage_callback = sgpiom_linkedsemi_manage_callback,
    // .sgpio_passthrough = sgpiom_linkedsemi_passthrough,
};

int sgpiom_linkedsemi_init(const struct device *device)
{
    return 0;
}
...
struct device_cont {
    const struct sgpiom_linkedsemi_config *cfg;
    struct sgpiom_linkedsemi_data *data;
};
...
#define SGPIOM_LINKEDSEMI_DT_DEFINE(node_id)            \
    DEVICE_DT_DEFINE(node_id,                           \
                     ...
                     &DT_PARENT(node_id).cfg[node_id],  \
                     POST_KERNEL,                       \
                     CONFIG_GPIO_INIT_PRIORITY,         \
                     &sgpiom_linkedsemi_driver);
