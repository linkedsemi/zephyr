...
static int pinctrl_configure_pin(const pinctrl_soc_pin_t pin_desc)
{
    ...
    IF_ENABLED(CONFIG_GPIO_CFG_LOCK, (io_cfg_lock(pin, false);))
    IF_ENABLED(CONFIG_GPIO_CFG_LOCK, (io_cfg_app_input_lock(pin, false);))
    IF_ENABLED(CONFIG_GPIO_CFG_LOCK, (io_func_cfg_lock(pin, false);))
    ...
    /* TODO: check */
    IF_ENABLED(CONFIG_GPIO_CFG_LOCK, (io_func_cfg_lock(pin, true);))
    IF_ENABLED(CONFIG_GPIO_CFG_LOCK, (io_cfg_lock(pin, true);))
    IF_ENABLED(CONFIG_GPIO_CFG_LOCK, (io_cfg_app_input_lock(pin, true);))
