...
#endif
        iopmp_config_region_napot4(dev, 2, 0x10070000, KB(64), true, true, true, false);
        iopmp_config_region_napot4(dev, 3, 0x10000000, KB(512), false, false, false, false);
        iopmp_config_region_napot4(dev, 4, SEC_SYSC_CPU_SEC_ADDR + 0x28 /* sec_cpu_intr */, 4, true, true, true, false);
...
static void c_delay(uint32_t us)
{
    uint32_t CLK_MHZ = 25;
    rv32_delay_asm((us) * (CLK_MHZ) >> 3, 1);
}
...
static void enable_dpll()
{
    ...
    while(1 != READ_BIT(SYSC_SEC_AWO->DPLL_LOCK, SYSC_SEC_AWO_DPLL1_LOCK_MASK));
    ...
    while(1 != READ_BIT(SYSC_SEC_AWO->DPLL_LOCK, SYSC_SEC_AWO_DPLL2_LOCK_MASK));
    // MODIFY_REG(SYSC_SEC_AWO->DPLL1_CTRL1, 0x7, 0x3);
    // MODIFY_REG(SYSC_SEC_AWO->DPLL2_CTRL1, 0x7, 0x3);
    // c_delay(100);
}

static void cpu_600M_ahb_300M_qspi_166M_init()
{
    // set_clk: CPU & AHB & flash & uart1_apb
    ...
    SYSC_SEC_AWO->PD_AWO_CLK_CTRL0 = 
                                 ...
                                 //| FIELD_BUILD(SYSC_SEC_AWO_HSE_DCT_EN, 0)
                                   | FIELD_BUILD(SYSC_SEC_AWO_HBUS_FLT_CTRL, 0x2)
                                   | FIELD_BUILD(SYSC_SEC_AWO_QSPI_FLT_CTRL, 0x2)
                                   | FIELD_BUILD(SYSC_SEC_AWO_CLK_SEL_QSPI, 0x1)
                                   | FIELD_BUILD(SYSC_SEC_AWO_CLK_SEL_HBUS_FLT, 0x9)
                                   | FIELD_BUILD(SYSC_SEC_AWO_CLK_SEL_QSPI_FLT, 0x9);
...
static void per_clock_select_divider_filter_init()
{
    /* SYSC_APP_AWO->PD_AWO_CLK_CTRL1 */
    MODIFY_REG(SYSC_APP_AWO->PD_AWO_CLK_CTRL1, SYSC_APP_AWO_CLK_SEL_PSRAM_MASK, 0x10); /* dpll_600M */
    MODIFY_REG(SYSC_APP_AWO->PD_AWO_CLK_CTRL1, SYSC_APP_AWO_CLK_SEL_PSRAM_FLT_MASK, 0x2); /* bypass */
    MODIFY_REG(SYSC_APP_AWO->PD_AWO_CLK_CTRL1, SYSC_APP_AWO_CLK_SEL_USB2_MASK, 0x4); /* dpll_48M */
    MODIFY_REG(SYSC_APP_AWO->PD_AWO_CLK_CTRL1, SYSC_APP_AWO_CLK_SEL_I3C_MASK, 0x4); /* clk_pbus4 */
    /* SYSC_APP_AWO->PD_AWO_CLK_CTRL1 */
    ...
    // MODIFY_REG(SYSC_APP_AWO->ETH1_CLK_CFG, SYSC_APP_AWO_ETH1_CLK_TX_DIV_MASK, 0x2); /* div = 2 */
    // MODIFY_REG(SYSC_APP_AWO->ETH1_CLK_CFG, SYSC_APP_AWO_ETH1_CLK_TX_SEL_MASK, 0x2); /* dpll 50M */
    // SET_BIT(SYSC_APP_AWO->ETH1_CLK_CFG, SYSC_APP_AWO_ETH1_CLK_TX_CG_MASK);
    // MODIFY_REG(SYSC_APP_AWO->ETH1_CLK_CFG, SYSC_APP_AWO_ETH1_CLK_RX_DIV_MASK, 0x2); /* div = 2 */
    // MODIFY_REG(SYSC_APP_AWO->ETH1_CLK_CFG, SYSC_APP_AWO_ETH1_CLK_RX_SEL_MASK, 0x2); /* dpll 50M */
    // SET_BIT(SYSC_APP_AWO->ETH1_CLK_CFG, SYSC_APP_AWO_ETH1_CLK_RX_CG_MASK);

    // MODIFY_REG(SYSC_APP_AWO->ETH2_CLK_CFG, SYSC_APP_AWO_ETH2_CLK_TX_DIV_MASK, 0x2); /* div = 2 */
    // MODIFY_REG(SYSC_APP_AWO->ETH2_CLK_CFG, SYSC_APP_AWO_ETH2_CLK_TX_SEL_MASK, 0x2); /* dpll 50M */
    // SET_BIT(SYSC_APP_AWO->ETH2_CLK_CFG, SYSC_APP_AWO_ETH2_CLK_TX_CG_MASK);
    // MODIFY_REG(SYSC_APP_AWO->ETH2_CLK_CFG, SYSC_APP_AWO_ETH2_CLK_RX_DIV_MASK, 0x2); /* div = 2 */
    // MODIFY_REG(SYSC_APP_AWO->ETH2_CLK_CFG, SYSC_APP_AWO_ETH2_CLK_RX_SEL_MASK, 0x2); /* dpll 50M */
    // SET_BIT(SYSC_APP_AWO->ETH2_CLK_CFG, SYSC_APP_AWO_ETH2_CLK_RX_CG_MASK);


    /* SYSC_APP_AWO->EMMC1_TX_RX_CLK */
    MODIFY_REG(SYSC_APP_AWO->EMMC1_TX_RX_CLK, SYSC_APP_AWO_EMMC1_CLK_TX_DIV_MASK, 0); /* div = 0 */
    MODIFY_REG(SYSC_APP_AWO->EMMC1_TX_RX_CLK, SYSC_APP_AWO_EMMC1_CLK_TX_SEL_MASK, 0x4); /* dpll 200M */
    SET_BIT(SYSC_APP_AWO->EMMC1_TX_RX_CLK, SYSC_APP_AWO_EMMC1_CLK_TX_CG_MASK);
    MODIFY_REG(SYSC_APP_AWO->EMMC1_TX_RX_CLK, SYSC_APP_AWO_EMMC1_CLK_RX_DIV_MASK, 0); /* div = 0 */
    MODIFY_REG(SYSC_APP_AWO->EMMC1_TX_RX_CLK, SYSC_APP_AWO_EMMC1_CLK_RX_SEL_MASK, 0x2); /* dpll 200M */
    SET_BIT(SYSC_APP_AWO->EMMC1_TX_RX_CLK, SYSC_APP_AWO_EMMC1_CLK_RX_CG_MASK);
    /* SYSC_APP_AWO->EMMC1_TX_RX_CLK */


    /* SYSC_APP_AWO->EMMC1_CORE_TIM_CLK */
    MODIFY_REG(SYSC_APP_AWO->EMMC1_CORE_TIM_CLK, SYSC_APP_AWO_EMMC1_CLK_CORE_DIV_MASK, 0); /* div = 0 */
    MODIFY_REG(SYSC_APP_AWO->EMMC1_CORE_TIM_CLK, SYSC_APP_AWO_EMMC1_CLK_CORE_SEL_MASK, 0x4); /* dpll 200M */
    SET_BIT(SYSC_APP_AWO->EMMC1_CORE_TIM_CLK, SYSC_APP_AWO_EMMC1_CLK_CORE_CG_MASK);
    MODIFY_REG(SYSC_APP_AWO->EMMC1_CORE_TIM_CLK, SYSC_APP_AWO_EMMC1_CLK_TIM_DIV_MASK, 0); /* div = 0 */
    MODIFY_REG(SYSC_APP_AWO->EMMC1_CORE_TIM_CLK, SYSC_APP_AWO_EMMC1_CLK_TIM_SEL_MASK, 0x2); /* dpll 200M */
    SET_BIT(SYSC_APP_AWO->EMMC1_CORE_TIM_CLK, SYSC_APP_AWO_EMMC1_CLK_TIM_CG_MASK);
    /* SYSC_APP_AWO->EMMC1_CORE_TIM_CLK */


    // /* SYSC_APP_AWO->EMMC2_TX_RX_CLK */
    // MODIFY_REG(SYSC_APP_AWO->EMMC2_TX_RX_CLK, SYSC_APP_AWO_EMMC2_CLK_TX_DIV_MASK, 0); /* div = 0 */
    // MODIFY_REG(SYSC_APP_AWO->EMMC2_TX_RX_CLK, SYSC_APP_AWO_EMMC2_CLK_TX_SEL_MASK, 0x4); /* dpll 200M */
    // SET_BIT(SYSC_APP_AWO->EMMC2_TX_RX_CLK, SYSC_APP_AWO_EMMC2_CLK_TX_CG_MASK);
    // MODIFY_REG(SYSC_APP_AWO->EMMC2_TX_RX_CLK, SYSC_APP_AWO_EMMC2_CLK_RX_DIV_MASK, 0); /* div = 0 */
    // MODIFY_REG(SYSC_APP_AWO->EMMC2_TX_RX_CLK, SYSC_APP_AWO_EMMC2_CLK_RX_SEL_MASK, 0x2); /* dpll 200M */
    // SET_BIT(SYSC_APP_AWO->EMMC2_TX_RX_CLK, SYSC_APP_AWO_EMMC2_CLK_RX_CG_MASK);
    // /* SYSC_APP_AWO->EMMC2_TX_RX_CLK */


    // /* SYSC_APP_AWO->EMMC2_CORE_TIM_CLK */
    // MODIFY_REG(SYSC_APP_AWO->EMMC2_CORE_TIM_CLK, SYSC_APP_AWO_EMMC2_CLK_CORE_DIV_MASK, 0); /* div = 0 */
    // MODIFY_REG(SYSC_APP_AWO->EMMC2_CORE_TIM_CLK, SYSC_APP_AWO_EMMC2_CLK_CORE_SE_MASKL, 0x4); /* dpll 200M */
    // SET_BIT(SYSC_APP_AWO->EMMC2_CORE_TIM_CLK, SYSC_APP_AWO_EMMC2_CLK_CORE_CG_MASK);
    // MODIFY_REG(SYSC_APP_AWO->EMMC2_CORE_TIM_CLK, SYSC_APP_AWO_EMMC2_CLK_TIM_DIV_MASK, 0); /* div = 0 */
    // MODIFY_REG(SYSC_APP_AWO->EMMC2_CORE_TIM_CLK, SYSC_APP_AWO_EMMC2_CLK_TIM_SEL_MASK, 0x2); /* dpll 200M */
    // SET_BIT(SYSC_APP_AWO->EMMC2_CORE_TIM_CLK, SYSC_APP_AWO_EMMC2_CLK_TIM_CG_MASK);
    // /* SYSC_APP_AWO->EMMC2_CORE_TIM_CLK */


    // MODIFY_REG(SYSC_APP_AWO->LPC_CLK, SYSC_APP_AWO_LPC1_CLK_DIV_MASK, 0x0); /* div = 0 */
    // MODIFY_REG(SYSC_APP_AWO->LPC_CLK, SYSC_APP_AWO_LPC1_CLK_SEL_MASK, 0x8); /* dpll 50M */
    // SET_BIT(SYSC_APP_AWO->LPC_CLK, SYSC_APP_AWO_LPC1_CLK_CG_MASK);

    // MODIFY_REG(SYSC_APP_AWO->LPC_CLK, SYSC_APP_AWO_LPC2_CLK_DIV_MASK, 0x0); /* div = 0 */
    // MODIFY_REG(SYSC_APP_AWO->LPC_CLK, SYSC_APP_AWO_LPC2_CLK_SEL_MASK, 0x8); /* dpll 50M */
    // SET_BIT(SYSC_APP_AWO->LPC_CLK, SYSC_APP_AWO_LPC1_CLK_CG_MASK);
}
...
void soc_early_init_hook(void)
{
    ...
    if (!is_app_cpu_running()) {
        ...
        per_clock_select_divider_filter_init();
    }
#endif
