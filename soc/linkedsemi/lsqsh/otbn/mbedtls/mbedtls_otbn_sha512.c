...
static void SHA512_BlockNumber_Update(uint32_t number)
{
    uint32_t n_checks = number;
    ls_otbn_dmem_write(SHA512_DMEM_N_CHUNKS_OFFSET, &n_checks, sizeof(uint32_t));
}

void ls_otbn_sha512_init_for_rtos()
{
    total_cnt = 0;
    remain_len = 0;
    sha_idx = SHA512_DMEM_MSG_OFFSET;
    ...
    ls_otbn_dmem_write(SHA512_DMEM_STATE_OFFSET, (uint32_t *)state_init, SHA512_DMEM_STATE_SIZE);
    ls_otbn_dmem_write(SHA512_DMEM_K_OFFSET, (uint32_t *)K, SHA512_DMEM_K_SIZE);
    ls_otbn_dmem_write(SHA512_DMEM_PTR_STATE_OFFSET, &state_ptr, sizeof(uint32_t));
    ls_otbn_dmem_write(SHA512_DMEM_PTR_MSG_OFFSET, &msg_ptr, sizeof(uint32_t));
...
static void msg_write(uint8_t *msg)
{
    ...
    ls_otbn_dmem_write(sha_idx, (uint32_t *)sha512_buffer, SHA512_BLOCK_SIZE);
    sha_idx += SHA512_BLOCK_SIZE;
    if (sha_idx == (SHA512_DMEM_MSG_SIZE + SHA512_DMEM_MSG_OFFSET))
    {
...
void ls_otbn_sha512_final_for_rtos(uint8_t result[SHA512_RESULT_SIZE])
{
    ...
    SHA512_BlockNumber_Update((sha_idx - SHA512_DMEM_MSG_OFFSET) / SHA512_BLOCK_SIZE);
    ls_otbn_cmd(OTBN_CMD_EXECUTE);

    uint64_t rs;
    for (uint8_t i = 0; i < 8; i++)
    {
        ls_otbn_dmem_read(SHA512_DMEM_STATE_OFFSET + i * 0x20, (uint32_t *)&rs, sizeof(uint64_t));
