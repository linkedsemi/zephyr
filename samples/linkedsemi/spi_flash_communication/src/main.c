...
#define FLASH_OFFSET      0x0000000      

static uint8_t data_write[] = {0x9f};  // The data to be written
static uint8_t data_read[1];  //Data for reading
...
{ 
    ...
    // Reading data
    rc = flash_read(dev, FLASH_OFFSET, data_read, len);
    if (rc != 0) {
