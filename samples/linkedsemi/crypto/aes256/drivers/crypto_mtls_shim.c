...
int mtls_ecb_encrypt(struct cipher_ctx *ctx, struct cipher_pkt *pkt)
{
	...
	for (int i = 0; i < pkt->in_len; i += 16) {
		ret = mbedtls_aes_crypt_ecb(ecb_ctx, MBEDTLS_AES_ENCRYPT,
						pkt->in_buf + i, pkt->out_buf + i);
		if (ret) {
...
int mtls_ctr_encrypt(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
	int ret;
	mbedtls_aes_context *ctr_ctx = MTLS_GET_CTX(ctx, aes);
	size_t nc_off = 0;
	uint8_t stream_block[16] = {};
	uint8_t nonce_counter[16] = {};

	memcpy(nonce_counter, iv, 16 - (ctx->mode_params.ctr_info.ctr_len >> 3));

	ret = mbedtls_aes_crypt_ctr(ctr_ctx, pkt->in_len, &nc_off, nonce_counter,
					stream_block, pkt->in_buf, pkt->out_buf);
	if (ret) {
		LOG_ERR("Could not encrypt (%d)", ret);
		return -EINVAL;
	}
			      ...
			      enum cipher_op op_type)
{
	...
	case CRYPTO_CIPHER_MODE_CTR:
		...
		ret = mbedtls_aes_setkey_enc(aes_ctx,
				ctx->key.bit_stream, ctx->keylen * 8U);
		ctx->ops.ctr_crypt_hndlr = mtls_ctr_encrypt;
		if (ret) {
