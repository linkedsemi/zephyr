...
const struct device *otbn_sm2_dev = DEVICE_DT_GET(DT_NODELABEL(sm2));
...
static struct sm2_ctx ctx_sm2;
static struct sm2_key key_sm2 =
{
	.d = private_k,
	.qx = public_x,
	.qy = public_y,
...
int main(void)
{
	...
	verify_result = sm2_verify(&ctx_sm2,&key_sm2,&pkt_sm2);
	if(verify_result == 0)
	{
		LOG_INF("sm2_verify, verify_result :succeed");
	}else
	{
		LOG_INF("sm2_verify, verify_result :fail");
	}
	return 0;
}