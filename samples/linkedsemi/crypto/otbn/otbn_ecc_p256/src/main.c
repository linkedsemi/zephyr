...
const struct device *otbn_ecc_p256_dev = DEVICE_DT_GET(DT_NODELABEL(ecc_p256));
...
static struct ecc_p256_ctx ctx_p256;
static struct ecc_p256_key key_p256 =
{
	.d = private_k,
	.qx = public_x,
	.qy = public_y,
...
int main(void)
{
	...
	verify_result = ecc_p256_verify(&ctx_p256,&key_p256,&pkt_p256);
	if(verify_result == 0)
	{
		LOG_INF("ecc_p256_verify, verify_result :succeed");
	}else
	{
		LOG_INF("ecc_p256_verify, verify_result :fail");
	}
	return 0;
}