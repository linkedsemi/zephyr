...
uint8_t kcs_buff[KCS_BUFF_SIZE];
...
void ipmi_cmd_handle(void *parameters, void *arvg0, void *arvg1)
{
	...
	case HOST_KCS_1: {
		...
		int rc = ipmi_write(ipmi_dev, kcs_buff, msg_cfg.buffer.data_len + 3);
		if (rc < 0 && (rc == -EINVAL || rc == -EPERM)) {
			LOG_ERR("Failed to write KCS data, rc = %d", rc);
		}
		LOG_DBG("resp ok");
		// kcs_write(msg_cfg.buffer.InF_source - HOST_KCS_1, kcs_buff, msg_cfg.buffer.data_len + 3);
		// SAFE_FREE(kcs_buff);
		break;
	}
#ifdef ENABLE_SSIF
...
void IPMI_handler(void *arug0, void *arug1, void *arug2)
{
	...
	while (1) {
		...
		LOG_DBG("IPMI_handler[%d]: netfn: %x", msg_cfg.buffer.data_len, msg_cfg.buffer.netfn);
