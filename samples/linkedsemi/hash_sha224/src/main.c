...
void sha224_mode(const struct device *dev,
                 ...
                 int *out_len)
{
    ...
    __ASSERT(ret == 0, "Failed to init sha224 session");
    ...
    do {
        ...
        // ret = memcmp(pkt.out_buf, sha224_results[9 - 1], 32);
        // __ASSERT(ret == 0, "Failed to compute hash for test");
        // printk("test match\n");
    } while (0);
...
void test_sha224(const struct device *sw_dev, const struct device *hw_dev)
{
    uint32_t out_len_sw;
    uint32_t out_len_hw;
