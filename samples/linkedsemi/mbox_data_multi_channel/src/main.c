...
int main(void)
{
	const struct mbox_dt_spec tx_channel0 = MBOX_DT_SPEC_GET(DT_NODELABEL(mbox_consumer0), tx);
	const struct mbox_dt_spec rx_channel0 = MBOX_DT_SPEC_GET(DT_NODELABEL(mbox_consumer0), rx);
	const struct mbox_dt_spec tx_channel1 = MBOX_DT_SPEC_GET(DT_NODELABEL(mbox_consumer1), tx);
	const struct mbox_dt_spec rx_channel1 = MBOX_DT_SPEC_GET(DT_NODELABEL(mbox_consumer1), rx);
	...
	while (message < 100) {
		...
		if (mbox_send_dt(&tx_channel0, &msg) < 0) {
			printk("mbox_send() error\n");
			return 0;
		}

		printk("Client send (on channel %d) value: %d\n", tx_channel1.channel_id, message);
		if (mbox_send_dt(&tx_channel1, &msg) < 0) {
			printk("mbox_send() error\n");
			return 0;
		}
